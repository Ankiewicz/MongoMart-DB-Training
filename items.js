...
function ItemDAO(database) {
    ...
    this.getCategories = function(callback) {
      ...
      database.collection('item').aggregate([
                          ...
                          // console.log("db returned results",db)
                            var total = 0
                          // totalling all categories items
                            for (var i = 0; i < db.length; i++) {
                              total = db[i].num + total
                            }
                            // pushing to db object doc
                            db.push({
                              _id: "All",
                              num: total
                            })
    ...
    this.getItems = function(category, page, itemsPerPage, callback) {
         ...
         *
         */
         var pageSkipItem = page * itemsPerPage;
    ...
    this.searchItems = function(query, page, itemsPerPage, callback) {
         ...
         *
         */
         var pageinationSkip = itemsPerPage * page;
         database.collection('item').aggregate([
                                  	{$match:  {$text:
                                  	    {
                                  	      $search: query,
    ...
    this.getItem = function(itemId, callback) {
         ...
         */
         ...
         database.collection('item').find({ _id: itemId}).toArray(function(err, db){
           console.log(db)
           var item = db[0]
            callback(item);
         })
